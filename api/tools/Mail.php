...
class Mail {
	...
	const TABLE_NAME = 'gf_emails';
	const FIELD_HASH = 'email_hash';
	const FIELD_DATE = 'email_date_send';
	const FIELD_TO = 'email_to';
	const FIELD_TITLE = 'email_title';
	const FIELD_MSG = 'email_message';
	const FIELD_HEADERS = 'email_headers';
	...
	public function Send() {
		...
		if($this->validation()) {
			...
			$dao = \PDOConnector::getInstance();
			
			$stat = $dao->prepare('INSERT INTO '.self::TABLE_NAME.' ('.self::FIELD_TO.', '.self::FIELD_TITLE.', '.self::FIELD_MSG.', '.self::FIELD_HEADERS.', '.self::FIELD_HASH.') VALUES (:to, :title, :msg, :headers, :hashid)');
	...
	private function getHeaders() {
		...
		(
			'Return-Path: ' . $from, 
			'X-Originating-IP: ' . $_SERVER['SERVER_NAME']
		);		
...
if (!function_exists('mb_encode_mimeheader')) { 
	function mb_encode_mimeheader($Text, $Delimiter = '%') 
	{ 
